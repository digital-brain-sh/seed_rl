...
ACTOR_BINARY="CUDA_VISIBLE_DEVICES='' python3 ../${ENVIRONMENT}/${ENVIRONMENT}_sampling.py --run_mode=actor";
LEARNER_BINARY="python3 ../${ENVIRONMENT}/${ENVIRONMENT}_sampling.py --run_mode=learner";
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"
NUM_ENVS=$(($NUM_ACTORS*$ENV_BATCH_SIZE))
CKPT_PATH="../${ENVIRONMENT}_experts/${SUB_TASK}/ckpt-${CKPT_ID}"
...
mkdir "/outdata/${ENVIRONMENT}_dataset"
